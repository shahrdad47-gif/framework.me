...
export function validateShortInput(data: Row, opts: { requireId: boolean }): string | null {
  ...
  if (data.notes !== undefined && data.notes !== null && typeof data.notes !== 'string') {
    return 'Notes must be a URL string'
  }
...
export async function getShortAdmin(id: string) {
  const sql = getSql()
  const rows = await sql`SELECT id, title, description, notes, status FROM shorts WHERE id = ${id} LIMIT 1`
...
export async function createShortAdmin(data: ShortInput) {
  ...
  const rows = await sql`
    INSERT INTO shorts (id, title, description, notes, status, display_order)
    VALUES (${data.id}, ${data.title}, ${sanitizeArticleBody(data.description ?? '')}, ${data.notes ?? null}, ${data.status}, ${maxOrder[0].m + 1})
    RETURNING id, title, description, notes, status
  `
