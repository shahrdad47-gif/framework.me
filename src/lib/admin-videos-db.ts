...
export function validateVideoInput(data: Row, opts: { requireId: boolean }): string | null {
  ...
  if (data.notes !== undefined && data.notes !== null && typeof data.notes !== 'string') return 'Notes must be a URL string'
...
export async function getVideoAdmin(id: string) {
  ...
  const rows = await sql`
    SELECT v.id, v.title, v.date, v.speaker, v.description, v.notes, v.category_id, v.status, c.label AS category_label
    FROM videos v JOIN video_categories c ON c.id = v.category_id
...
export async function createVideoAdmin(data: VideoInput) {
  ...
  const rows = await sql`
    INSERT INTO videos (id, title, date, speaker, description, notes, category_id, status, display_order)
    VALUES (
      ${data.id}, ${data.title}, ${data.date ?? null}, ${data.speaker ?? null},
      ${sanitizeArticleBody(data.description ?? '')}, ${data.notes ?? null}, ${data.categoryId}, ${data.status}, ${maxOrder[0].m + 1}
    )
    RETURNING id, title, date, speaker, description, notes, category_id, status
  `
